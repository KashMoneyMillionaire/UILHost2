...
namespace UILHost.Web.Controllers
{
    ...
    public partial class ManageController : Controller
    {
        ...
        public ManageController(SessionFacade session, IAuthenticationService authenticationSvc, IUnitOfWorkAsync uow)
        {
            _session = session;
        ...
        public virtual ActionResult ChangePassword(ChangePasswordViewModel model)
        {
            ...
            var profile = _uow.Repository<Teacher>().Find(_session.CurrentTeacher.Id);

            if (profile.VerifyPassword(model.OldPassword))
            {
                profile.SetPassword(model.NewPassword);
                _authenticationSvc.SignOut();
                _authenticationSvc.SignIn(profile, true);
                _uow.Repository<Teacher>().Update(profile);
                _uow.SaveChanges();

                return RedirectToAction(MVC.Manage.Index(ManageMessageId.ChangePasswordSuccess));
            }

            ModelState.AddModelError("error", "The old password you entered does not match our records.");
            return View(model);
        }
